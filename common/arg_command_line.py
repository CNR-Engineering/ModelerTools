...
class myargparse(argparse.ArgumentParser):
    ...
    """
    def __init__(self, add_args=[], lang='en', *args, **kwargs):
        ...
        for arg in add_args:
            ...
            if arg == 'force':
                if lang=='en':
                    ...
                    self.add_argument("--force", "-f", help="Force l'écriture du fichier de sortie", action="store_true")

            elif arg == 'verbose':
                if lang=='en':
                    self.add_argument("--verbose", "-v", help="increase output verbosity", action="store_true")
                elif lang=='fr':
                    self.add_argument("--verbose", "-v", help="Augmenter la verbosité", action="store_true")
